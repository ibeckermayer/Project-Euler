...
def prodDown(grid,i,j):
    if i+3<rows:
        return grid[i][j]*grid[i+1][j]*grid[i+2][j]*grid[i+3][j]
    else: 
        return 0
        
def prodUpRight(grid,i,j):
    if i-3>=0 and j+3<columns:
        return grid[i][j]*grid[i-1][j+1]*grid[i-2][j+2]*grid[i-3][j+3]
    else:
        return 0
        
def prodUpLeft(grid,i,j):
    if i-3>=0 and j-3>=0:
       return grid[i][j]*grid[i-1][j-1]*grid[i-2][j-2]*grid[i-3][j-3]
    else:
        return 0 
        
def prodDownRight(grid,i,j):
    if i+3<rows and j+3<columns:
        return grid[i][j]*grid[i+1][j+1]*grid[i+2][j+2]*grid[i+3][j+3]
    else:
        return 0

def prodDownLeft(grid,i,j):
    if i+3<rows and j-3>=0:
        return grid[i][j]*grid[i+1][j-1]*grid[i+2][j-2]*grid[i+3][j-3]
    else:
        return 0
        
def getProductsOfAllAdjacent4s(grid,i,j):
    return [prodRight(grid,i,j),prodLeft(grid,i,j),prodUp(grid,i,j),prodDown(grid,i,j),prodUpRight(grid,i,j),prodUpLeft(grid,i,j),prodDownRight(grid,i,j),prodDownLeft(grid,i,j)]
...
productsOfFourAdjacentsInGrid = []

for i in xrange(rows):
    for j in xrange(columns):
        prods = getProductsOfAllAdjacent4s(grid,i,j)
        for prod in prods:
            productsOfFourAdjacentsInGrid.append(prod)
            
print findGreatest(productsOfFourAdjacentsInGrid)      
